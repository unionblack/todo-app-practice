...
function render_item (item, model, signal) {
  ...
  // Создаём элементы с помощью elmish
  return (
    li(liAttrs, [
      div(["class=view"], [
        input([
...
function render_main (model, signal) {
  // Если задач нет — скрываем секцию
  var display = "style=display:" + (model.todos && model.todos.length > 0 ? "block" : "none");

  // Фильтруем задачи в зависимости от выбранного маршрута (hash)
  var filteredTodos = (model.todos && model.todos.length > 0) ?
    model.todos.filter(function (item) {
      switch(model.hash) {
        case '#/active':
          return !item.done;      // только активные (не выполненные)
        case '#/completed':
          return item.done;       // только выполненные
        default:
          return item;            // все задачи
      }
    }) : [];
...
function render_footer (model, signal) {
  // Считаем количество активных и выполненных задач
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    model: initial_model,
    update: update,
    render_item: render_item,      // <-- добавьте эту строку
    render_main: render_main,      // <-- добавьте эту строку
    render_footer: render_footer   // <-- добавьте эту строку
  }
}